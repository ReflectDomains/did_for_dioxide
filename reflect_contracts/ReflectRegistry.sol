...
contract ReflectRegistry is Reflect {
    ...
    /**
     * @dev Constructs a new Reflect registrar.
     */
    constructor(address _lauchTld) public {
        records[0x0].owner = _lauchTld;
    }
    ...
    function setRecord(bytes32 node, address owner, address resolver, uint64 ttl, bool permanent) external {
        setOwner(node, owner);
        _setResolverAndTTL(node, resolver, ttl, permanent);
    }
    ...
    function setSubnodeRecord(bytes32 node, bytes32 label, address owner, address resolver, uint64 ttl, bool permanent) external {
        ...
        _setResolverAndTTL(subnode, resolver, ttl, permanent);
    }
    ...
    function setPermanent(bytes32 node, bool permanent) public authorised(node) {
        emit SetPermanent(node, permanent);
        records[node].permanent = permanent;
    }

    /**
     * @dev Enable or disable approval for a third party ("operator") to manage
     *  all of `msg.sender`'s Reflect records. Emits the ApprovalForAll event.
    ...
    function permanent(bytes32 node) public view returns (bool) {
        return records[node].permanent;
    }
    ...
    function _setResolverAndTTL(bytes32 node, address resolver, uint64 ttl, bool permanent) internal {
